...
export const VideoDownloaderWorkspace: React.FC<VideoDownloaderWorkspaceProps> = ({
  ...
  const handleConvertToMp3 = async () => {
    ...
    try {
      ...
      const audioSource = videoInfo.audioUrl || videoInfo.videoUrl;
      ...
      onShowToast({
        type: 'success',
        message: 'Audio extracted and downloaded as MP3 successfully!',
      });
    } catch (err: any) {
      onShowToast({
        type: 'error',
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="pt-2 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => handleDownloadVideo(videoInfo.streams?.find(s => s.quality === '1080p')?.url || videoInfo.videoUrl, '1080p_FullHD')}
                  ...
                  <button
                    type="button"
                    onClick={() => handleDownloadVideo(videoInfo.streams?.find(s => s.quality === '720p')?.url || videoInfo.videoUrl, '720p_HD')}
                    disabled={!!downloadingFormat}
                ...
                {/* Additional on-site quality stream options if available */}
                {videoInfo.streams && videoInfo.streams.filter(s => s.format !== 'mp3' && s.quality !== '1080p' && s.quality !== '720p').length > 0 && (
                  <div className="pt-2">
                    <div className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2">
                      Other Available Resolutions (Direct On-Site):
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {videoInfo.streams
                        .filter(s => s.format !== 'mp3' && s.quality !== '1080p' && s.quality !== '720p')
                        .map((stream, idx) => (
                          <button
                            key={idx}
                            type="button"
                            onClick={() => handleDownloadVideo(stream.url, stream.quality)}
                            disabled={!!downloadingFormat}
                            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold transition-colors cursor-pointer"
                          >
                            <Download className="w-3.5 h-3.5 text-brand-600 dark:text-brand-400" />
                            <span>{stream.label || `${stream.quality} MP4`}</span>
                          </button>
                        ))}
                    </div>
                  </div>
